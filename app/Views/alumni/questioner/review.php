...
<div class="container mt-4">
                    ...
                    <?php foreach ($section['questions'] as $q): ?>
                        <div class="mb-3">
                            ...
                            <?php elseif (in_array(strtolower($q['question_type']), ['scale', 'matrix_scale'])): ?>
                                <p class="form-control-static"><?= esc($answer ?: 'Belum dijawab') ?> (Skala: <?= esc($q['scale_min'] ?? 1) ?> - <?= esc($q['scale_max'] ?? 10) ?>)</p>
                            ...
                            <?php elseif (strtolower($q['question_type']) === 'file'): ?>
                                <p class="form-control-static">
                                    <?php if ($answer && strpos($answer, 'uploaded_file:') === 0): ?>
                                        <a href="<?= base_url(str_replace('uploaded_file:', 'uploads/answers/', $answer)) ?>" target="_blank">Lihat file: <?= esc(basename($answer)) ?></a>
                                    <?php else: ?>
    ...
    <a href="<?= base_url('alumni/questionnaires/mulai/' . $q_id) ?>" class="btn btn-primary">Kembali ke Form</a>
    <a href="<?= base_url('alumni/questionnaires') ?>" class="btn btn-secondary">Kembali ke Daftar</a>
</div>
<?= $this->endSection() ?>